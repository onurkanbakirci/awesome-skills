              ...
              <Card key={index} className="border-[#eaeaea]">
                <CardHeader>
                  <div className="flex items-start gap-3 mb-2">
                    ...
                    <code className="text-base font-mono text-black flex-1 mt-0.5">
                      {endpoint.path}
                    </code>
                  </div>
                  <CardDescription className="text-[#666] text-base">
                ...
                <CardContent className="space-y-4">
                  ...
                  {endpoint.parameters && endpoint.parameters.length > 0 && (
                    <div>
                      <h3 className="text-sm font-semibold text-black mb-2">Parameters:</h3>
                      <div className="space-y-2">
                        {endpoint.parameters.map((param, paramIndex) => (
                          <div key={paramIndex} className="flex items-start gap-2 text-sm">
                            <code className="px-2 py-0.5 bg-[#f5f5f5] rounded text-black font-mono">
                              {param.name}
                            </code>
                            <span className="text-[#999]">({param.type})</span>
                            {param.required && (
                              <span className="px-1.5 py-0.5 bg-red-100 text-red-700 text-xs rounded">
                                required
                              </span>
                            )}
                            <span className="text-[#666] flex-1">- {param.description}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {/* Response */}
                  <div>
                    <h3 className="text-sm font-semibold text-black mb-2">Response:</h3>
                    {typeof endpoint.response.type === 'string' ? (
                      <div className="text-sm text-[#666]">
                        <span className="font-mono text-black">{endpoint.response.type}</span>
                        {endpoint.response.description && ` - ${endpoint.response.description}`}
                      </div>
                    ) : (
                      <div className="bg-[#f5f5f5] rounded p-3 border border-[#d4d4d4]">
                        <pre className="text-xs font-mono text-black overflow-x-auto">
                  ...
                  {/* Examples */}
                  <div>
                    <h3 className="text-sm font-semibold text-black mb-2">Examples:</h3>
                        ...
                        <div key={exampleIndex} className="space-y-2">
                          ...
                          <div className="bg-[#f5f5f5] rounded p-3 border border-[#d4d4d4] flex items-center justify-between gap-2">
                            ...
                            <a
                              ...
                              className="flex-shrink-0 px-3 py-1 text-xs text-white bg-black hover:bg-[#333] rounded transition-colors"
